...
```bash
docker commit <container_name> hazoe/rest-demo:v1
```
> Creates a new Docker image named `hazoe/rest-demo` with tag `v1` from the current container state.
...
docker images
```
> Verifies that `hazoe/rest-demo:v1` image has been created successfully.


### Default Behavior: JShell

When running hazoe/rest-demo:v1, the container defaults to JShell:

```bash
docker run hazoe/rest-demo:v1
```
...
```bash
docker commit `
  --change='CMD java -jar /tmp/rest-demo.jar' `
  <container_name> hazoe/rest-demo:v2
```
...
```bash
docker run -p 8081:8080 hazoe/rest-demo:v2
```
> Maps port `8080` of the container to `8081` on the host machine.